    ...
    public partial class Inheritance : DbMigration
    {
        public override void Up()
        {
            ...
            Sql("INSERT INTO dbo.Person (LastName, FirstName, HireDate, EnrollmentDate, Discriminator, OldId) " +
                "SELECT LastName, FirstName, null AS HireDate, EnrollmentDate, 'Student' AS Discriminator, ID " +
                "AS OldId FROM dbo.Student");
            
            // Fix up existing relationships to match new PK's.
            Sql("UPDATE dbo.Enrollment SET StudentId = (SELECT ID FROM dbo.Person WHERE " +
                "OldId = Enrollment.StudentId AND Discriminator = 'Student')");
